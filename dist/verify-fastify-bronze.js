...
export function verifyComposedFastifyBronze(outDir, compose, expectedHandlers) {
    const checks = [];
    checks.push({
...
export function ensureComposedFastifyDeps(outDir) {
    const pkg = join(outDir, "package.json");
    if (!existsSync(pkg))
        return;
    if (existsSync(join(outDir, "node_modules", "fastify")))
        return;
...
export async function verifyComposedFastifyRuntime(outDir, routes) {
    const checks = [];
    try {
