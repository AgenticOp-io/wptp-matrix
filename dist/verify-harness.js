...
import { runSilverEchoApiIrNextJsChrysalis, runSilverHarIrHonoChrysalis, runSilverHarIrNextJsChrysalis, runSilverOpenApiIrHonoChrysalis, runSilverOpenApiIrNextJsChrysalis, } from "./verify-silver-chrysalis.js";
...
export const HARNESS_CASES = [
    ...
    {
        id: "openapi-ir-nextjs-chrysalis",
        grade: "silver",
        description: "OpenAPI → IR → WebIR → @wptp/emit-nextjs via Chrysalis bundle bridge",
    },
    {
...
export async function runMatrixHarness(options) {
    ...
    const emitNextJsRoot = process.env.WPTP_EMIT_NEXTJS_ROOT;
    results.push(runSilverOpenApiIrNextJsChrysalis(join(root, "petstore-mini.openapi.json"), process.env.CHRYSALIS_ROOT, emitNextJsRoot));
    results.push(runSilverEchoApiIrNextJsChrysalis(join(root, "echo-api.openapi.json"), process.env.CHRYSALIS_ROOT, emitNextJsRoot));
    results.push(runSilverHarIrNextJsChrysalis(join(root, "mini.har.json"), process.env.CHRYSALIS_ROOT, emitNextJsRoot));
