...
} from "./verify-silver-chrysalis.js";
import {
...
export const HARNESS_CASES: ReadonlyArray<HarnessCase> = [
  ...
  {
    id: "echo-api-ir-hono-chrysalis",
    grade: "silver",
    description: "Echo OpenAPI → IR → WebIR → Chrysalis emit-hono (silver lowering)",
  },
...
export async function runMatrixHarness(options: {
  ...
  results.push(
    runSilverHarIrNextJsChrysalis(join(root, "mini.har.json"), process.env.CHRYSALIS_ROOT, emitNextJsRoot),
  );
