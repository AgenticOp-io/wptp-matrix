...
export function irPathToFetchPath(path: string): string {
  return path.replace(/\{[^}]+\}/g, "1");
}

/** Install emitted package.json deps so `import "hono"` resolves under outDir. */
export function ensureComposedHonoDeps(outDir: string): void {
  const pkg = join(outDir, "package.json");
  if (!existsSync(pkg)) return;
  if (existsSync(join(outDir, "node_modules", "hono"))) return;
...
export async function verifyComposedHonoRuntime(
  outDir: string,
  ...
  try {
    ...
    for (const route of routes) {
      ...
      const body = (await res.json()) as { ok?: boolean; route?: string; body?: unknown };
      checks.push({
